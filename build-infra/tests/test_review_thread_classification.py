...
def test_sixth_classification_preserves_evidence_partition() -> None:
    ...
    assert sum(row["completion_state"] == "actionable" for row in rows) == 29
    assert sum(row["completion_state"] == "pending-evidence" for row in rows) == 2
    assert sum(row["completion_state"] == "closed" for row in rows) == 19
    assert sum(row["disposition"] == "fixed" for row in rows) == 14
    assert sum(row["disposition"] == "superseded" for row in rows) == 5
    assert sum(row["resolution_state"] == "resolved" for row in rows) == 19
    assert sum(row["resolution_state"] == "unresolved" for row in rows) == 31
