...
FIFTH_CAPTURE_PATH = (
    BUILD_INFRA_ROOT
    / "docs/review-thread-frontiers/merged-thread-frontier-after-TvpLc/frontier.tsv"
)
FIFTH_ASSESSMENT_PATH = (
    BUILD_INFRA_ROOT
    / "docs/review-thread-classifications/merged-thread-frontier-after-TvpLc/assessments.tsv"
)
FIFTH_OUTPUT_PATH = (
    BUILD_INFRA_ROOT
...
def fifth_retained_inputs() -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    capture_rows = review_thread_classification.read_tsv(
        FIFTH_CAPTURE_PATH, review_thread_classification.CAPTURE_FIELDS
    )
    assessment_rows = review_thread_classification.read_tsv(
        FIFTH_ASSESSMENT_PATH, review_thread_classification.ASSESSMENT_FIELDS
    )
    return capture_rows, assessment_rows


def build_fifth_rows(
    capture_rows: list[dict[str, str]], assessment_rows: list[dict[str, str]]
) -> list[dict[str, str]]:
    return review_thread_classification.build_rows(
        capture_rows,
        assessment_rows,
        "merged-review-thread-closure-batch-0005",
        50,
    )
...
def test_fifth_classification_matches_generated_frontier() -> None:
    capture_rows, assessment_rows = fifth_retained_inputs()
    expected = build_fifth_rows(capture_rows, assessment_rows)
    retained = review_thread_classification.read_tsv(
        FIFTH_OUTPUT_PATH, review_thread_classification.FRONTIER_FIELDS
    )
    assert retained == expected


def test_fifth_classification_rejects_missing_assessment() -> None:
    capture_rows, assessment_rows = fifth_retained_inputs()
    with pytest.raises(
        review_thread_classification.FrontierError, match="membership differs"
    ):
...
def test_sixth_classification_preserves_evidence_partition() -> None:
    ...
    assert sum(row["completion_state"] == "actionable" for row in rows) == 25
    assert sum(row["completion_state"] == "pending-evidence" for row in rows) == 2
    assert sum(row["completion_state"] == "closed" for row in rows) == 23
    assert sum(row["disposition"] == "fixed" for row in rows) == 18
    assert sum(row["disposition"] == "superseded" for row in rows) == 5
    assert sum(row["resolution_state"] == "resolved" for row in rows) == 23
    assert sum(row["resolution_state"] == "unresolved" for row in rows) == 27
