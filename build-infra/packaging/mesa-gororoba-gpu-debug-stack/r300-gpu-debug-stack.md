...
|---|---|---|---|---|---|
| **1_** | **prime** | `mesa-gororoba-debug-prime` | `1_r300_full_debug_prime_asan_o0` | **scoped /opt, via wrapper** | ASan+UBSan+extended sanitizers, -O0; the highest-information forensic RCA |
...
(2_)** tiers stage ONLY to `/opt/mesa-26-gororoba-debug-prime` and
...
`1_r300_full_debug_prime_asan_o0` = the full r300 surface + `b_sanitize=address,undefined`
...
## Running the prime driver (the wrapper)
    ...
    mesa-26-gororoba-debug-prime-run vkcube
...
- **`RADEON_DEBUG`** (free, built-in, the single most useful): `fp` dumps the r300
  ...
  `vm` the virtual-memory map.  `RADEON_DEBUG=fp,cs mesa-26-gororoba-debug-prime-run <app>`.
...
version, 26.2.0) and `epoch` (2) and are rebuilt together:
    ...
    make rebuild-gpu-debug-stack    # rebuild all four r300 tier builddirs from the current tree

Then repack each (`for p in mesa-gororoba-debug-prime mesa-gororoba-debug-o0
mesa-gororoba-debug-optimized mesa-gororoba umr-gororoba mesa-gororoba-gpu-debug-stack;
...
`mesa-gororoba-gpu-debug-stack` depends on the prime driver + umr + tools and
