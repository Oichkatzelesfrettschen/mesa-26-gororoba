...
for tool in meson ninja pkg-config python3 clang-22 clang++-22 ccache distcc; do
    need_command "$tool"
done
...
if command -v sccache >/dev/null 2>&1; then
    ...
    warn "missing command sccache; Rust builds will run without the configured cache wrapper"
fi

if command -v rustc >/dev/null 2>&1; then
    ok "command rustc"
else
    warn "missing command rustc; required only when Meson enables Rust targets"
fi

if [ "$RUSTICL_ENABLED" -eq 1 ]; then
    need_command sccache
    need_command rustc
fi
...
if [ "$RUSTICL_ENABLED" -eq 1 ]; then
    ...
    if command -v llvm-config-22 >/dev/null 2>&1; then
        ok "command llvm-config-22"
    elif command -v llvm-config >/dev/null 2>&1; then
        ok "command llvm-config"
    else
        warn "llvm-config not found; confirm LLVM and libclang development files manually"
    fi
else
    ok "profile disables Rusticl for Terakan daily lane"
fi

printf '\nPackage hints for missing daily-lane tools:\n'
printf '  meson ninja clang-22 pkg-config ccache distcc sccache rust\n'
...
if [ "$RUSTICL_ENABLED" -eq 1 ]; then
    printf '  rust-bindgen llvm libclang\n'
fi
