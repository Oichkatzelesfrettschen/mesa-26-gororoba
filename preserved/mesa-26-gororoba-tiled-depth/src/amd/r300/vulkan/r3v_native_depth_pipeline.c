...
lower_compare(VkCompareOp op, uint32_t *function)
{
   ...
   if (function == NULL || op > VK_COMPARE_OP_ALWAYS)
      return false;
...
{
   if (state == NULL || shader == NULL || out == NULL || state->flags != 0 ||
       ...
       state->back.depthFailOp != VK_STENCIL_OP_KEEP || depth_bias_enabled)
      return -EINVAL;
   ...
   struct r3v_native_depth_pipeline_state candidate = {
      .hardware = {
         ...
         .depth_write = state->depthTestEnable &&
