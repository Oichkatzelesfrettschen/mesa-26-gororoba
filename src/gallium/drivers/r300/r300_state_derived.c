...
static void r300_update_rs_block(struct r300_context *r300)
{
     ...
     * RS/PSC from LOAD_VBPNTR.  The route belongs to the screen; the separate
     * execution experiment remains a process option cached on this hot path. */
    {
        static int r2vb_exec_gate = -1;
        if (r2vb_exec_gate < 0) {
            const char *r2vb_exec = getenv("R300_R2VB_EXEC");
            r2vb_exec_gate =
                r2vb_exec && strcmp(r2vb_exec, "1") == 0 ? 1 : 0;
        }
        const bool r2vb_on =
            r300_screen_r2vb_config(r300->screen)->route_enabled ||
            r2vb_exec_gate == 1;
