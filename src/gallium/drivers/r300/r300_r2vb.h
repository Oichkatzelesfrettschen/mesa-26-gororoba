 ...
 * A hazard-gated PVS-bank proof lane would replace only that stage-1 producer;
 * the barrier and the re-ingest/oracle half stay the same.
 *
 ...
 * VAP fetch (stage 3) rather than only the stage-1 render.  Pass NULL,0,0 for a
 * single-BO loop.
 *
 * This sequence issues raw PM4 vertex/draw packets; on the RS482 hazard lane a
 * live submit is gated behind explicit raw-submit consent.  The CB-write ->
 * barrier -> vertex-fetch data path is coherency-validated through the GL
 * oracle; the Gallium Draw-free direct-VAP timing is the remaining
 * hazard-gated measurement. */
