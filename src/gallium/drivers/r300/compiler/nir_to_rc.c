...
/* RC scalar math opcodes replicate one source channel across the written
 * destination components, so vector NIR ops that use them are emitted one
 * component at a time. */
static void
          ...
          union r300_shader_code rc, struct radeon_compiler *compiler)
{
    ...
    * to umod/udiv, which nir_lower_int_to_float lowers.  Reject unsupported
    * bitwise operations and non-boolean integer widths other than 32 bits to a
    * dummy shader before int_to_float can assert on them. */
