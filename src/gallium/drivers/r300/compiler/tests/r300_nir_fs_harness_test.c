 ...
 * operation identified by the fog SSA-temp mistranslation.  All operands are
 * varyings so nothing constant-folds and the full packing reaches the emitter,
 * matching the fog program whose RS482 RC dump read never-written temporaries
 * and wrote the RC sentinel temp[RC_REGISTER_MAX_INDEX - 1]. */
static nir_shader *
build_fs_fog(void)
{
   static const nir_shader_compiler_options options = {
