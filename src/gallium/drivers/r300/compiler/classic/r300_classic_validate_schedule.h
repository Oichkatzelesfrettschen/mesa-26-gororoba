 ...
 *     always pairs with an RGB lane computing the same dot product, since
 *     Alpha's DP mode broadcasts RGB's result rather than computing its own.
 *   - WriteALUResult and an output write are never both set on one pair
 *     (they share encoding bits on both targets).
 ...
 *
 * On the first violation, returns false and sets *reject_reason to a
 * static string naming the constraint that failed. */
bool
