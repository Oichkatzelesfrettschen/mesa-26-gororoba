...
unsigned int
combine_swizzles4(unsigned int src, rc_swizzle swz_x, rc_swizzle swz_y, rc_swizzle swz_z,
                  rc_swizzle swz_w)
{
   unsigned int ret = 0;
...
rewrite_writemask(unsigned int old_mask, unsigned int conversion_swizzle)
{
   ...
   for (i = 0; i < 4; i++) {
      if (!GET_BIT(old_mask, i) || GET_SWZ(conversion_swizzle, i) == RC_SWIZZLE_UNUSED) {
         continue;
      }
 ...
 */
void
rc_pair_rewrite_writemask(struct rc_pair_sub_instruction *sub, unsigned int conversion_swizzle)
{
   ...
   for (i = 0; i < info->NumSrcRegs; i++) {
      sub->Arg[i].Swizzle = rc_adjust_channels(sub->Arg[i].Swizzle, conversion_swizzle);
   }
}
 ...
 */
void
rc_normal_rewrite_writemask(struct rc_instruction *inst, unsigned int conversion_swizzle)
{
   struct rc_sub_instruction *sub = &inst->U.I;
   const struct rc_opcode_info *info = rc_get_opcode_info(sub->Opcode);
   sub->DstReg.WriteMask = rewrite_writemask(sub->DstReg.WriteMask, conversion_swizzle);
   ...
   rc_for_all_reads_src(inst, normal_rewrite_writemask_cb, &conversion_swizzle);
}
...
rc_source_type_swz(unsigned int swizzle)
{
   ...
   for (chan = 0; chan < 4; chan++) {
      ...
      if (swz == RC_SWIZZLE_W) {
         ret |= RC_SOURCE_ALPHA;
      } else if (swz == RC_SWIZZLE_X || swz == RC_SWIZZLE_Y || swz == RC_SWIZZLE_Z) {
         ret |= RC_SOURCE_RGB;
      }
   }
   return ret;
...
static void
can_use_presub_data_add_select(struct can_use_presub_data *data, rc_register_file file,
 ...
 */
static void
can_use_presub_read_cb(void *userdata, struct rc_instruction *inst, struct rc_src_register *src)
{
                       ...
                       const struct rc_src_register *presub_src1)
{
   ...
   can_use_presub_data_add_select(&d, presub_src0->File, presub_src0->Index, presub_src0->Swizzle);

   if (num_presub_srcs > 1) {
      src_type1 = rc_source_type_swz(presub_src1->Swizzle);
      can_use_presub_data_add_select(&d, presub_src1->File, presub_src1->Index,
                                     presub_src1->Swizzle);
       ...
       */
      if (presub_src0->File == presub_src1->File && presub_src0->Index == presub_src1->Index) {
   ...
   for (i = 0; i < d.SelectCount; i++) {
      ...
      for (j = i + 1; j < d.SelectCount; j++) {
         ...
         if (!c->is_r500 && (!r300_swizzle_is_native_basic(d.Selects[i].Swizzle) ||
                             !r300_swizzle_is_native_basic(d.Selects[j].Swizzle)))
            continue;
         if (d.Selects[i].File == d.Selects[j].File && d.Selects[i].Index == d.Selects[j].Index) {
...
static void
max_callback(void *userdata, struct rc_instruction *inst, rc_register_file file, unsigned int index,
             unsigned int mask)
{
 ...
 */
void
rc_pair_remove_src(struct rc_instruction *inst, unsigned int src_type, unsigned int source)
{
   if (src_type & RC_SOURCE_RGB) {
      memset(&inst->U.P.RGB.Src[source], 0, sizeof(struct rc_pair_instruction_source));
   }

   if (src_type & RC_SOURCE_ALPHA) {
      memset(&inst->U.P.Alpha.Src[source], 0, sizeof(struct rc_pair_instruction_source));
   }
}
 ...
 */
float
rc_get_constant_value(struct radeon_compiler *c, unsigned int index, unsigned int swizzle,
