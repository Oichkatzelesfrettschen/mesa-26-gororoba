                                       ...
                                       uint32_t stage3_height, bool transform_mode)
{
     ...
     * gate adds seven; the C1c gate adds three; the C2 gate adds five (SEQ-of-
     * 4 + header); the C3 gate adds eight (SEQ-of-7 + header); the C4 gate
     * adds five (3 single writes = 6 dwords... wait three OUT_CS_REG = 6
     * dwords, but interleaved adjacent ones could pack -- here they are not
     * contiguous so three separate writes = 6 dwords). */
