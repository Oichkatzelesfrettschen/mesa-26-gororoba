...
RatInstr::emit_uav_store_r600(nir_intrinsic_instr *intr, Shader& shader)
{
   ...
   rat_id += shader.ssbo_image_offset();
   ...
   if (uav_store_fuzz_field && uav_store_fuzz_field[0]) {
      ...
      if (!strcmp(uav_store_fuzz_field, "comp_mask") &&
         ...
         if (uav_store_fuzz_value == 0) {
             ...
             * caller gets a clear diagnostic instead of a silent no-op dispatch. */
            fprintf(stderr,
    ...
    * forwards comp_mask without clamping to let deliberate non-baseline values
   ...
   if (uav_returning_fuzz_field && uav_returning_fuzz_field[0]) {
      ...
      if (!strcmp(uav_returning_fuzz_field, "comp_mask") &&
         ...
         if (uav_returning_fuzz_value == 0) {
             ...
             * written; reject it so the caller gets a clear diagnostic instead
             * of a silent no-op dispatch (no clamp exists on this path). */
            fprintf(stderr,
                    "TERAKAN_FUZZ_FIELD=comp_mask TERAKAN_FUZZ_VALUE=0 "
      ...
      if (raw && raw[0]) {
         ...
         if (!strcmp(fuzz_field, "comp_mask") &&
            ...
            if (fuzz_value == 0) {
                ...
                * diagnostic instead of a silent no-op dispatch (no clamp exists
                * on this path). */
               fprintf(stderr,
                       "TERAKAN_FUZZ_FIELD=comp_mask TERAKAN_FUZZ_VALUE=0 "
