...
private:
   ...
   int apply_pv_ps_to_group(AluGroup& group, AluGroup& prev_group);
   int apply_pv_ps_to_instr(AluGroup& group,
                            AluInstr *prev,
   ...
   int m_pv_total_groups{0};
   int m_pv_groups_with_pv{0};
   int m_pv_total_substitutions{0};
};
...
void
BlockScheduler::report_pv_chain_stats()
{
   if (m_pv_total_groups == 0)
      return;

   float pv_ratio = m_pv_total_groups > 0
      ? 100.0f * m_pv_groups_with_pv / m_pv_total_groups
      : 0.0f;

   sfn_log << SfnLog::schedule
           << "PV-chain stats: "
           << m_pv_total_groups << " groups, "
           << m_pv_groups_with_pv << " with PV/PS ("
           << pv_ratio << "%), "
...
find_next_addr_load(const AluInstr& addr_load, const Register& addr)
{
   ...
   return next_addr_load;
}

static std::list<AluInstr *>
...
void BlockScheduler::maybe_split_alu_block(Shader::ShaderBlocks& out_blocks)
{
   ...
   for (auto instr : *m_current_block) {
      ...
      if (prev_group) {
         int subs = apply_pv_ps_to_group(*group, *prev_group);
         m_pv_total_groups++;
         m_pv_total_substitutions += subs;
         if (subs > 0) {
            m_pv_groups_with_pv++;
            m_pv_current_chain++;
            if (m_pv_current_chain > m_pv_max_chain)
               m_pv_max_chain = m_pv_current_chain;
         } else {
            m_pv_current_chain = 0;
         }
      } else {
         m_pv_current_chain = 0;
      }
...
BlockScheduler::apply_pv_ps_to_group(AluGroup& group, AluGroup& prev_group)
{
   ...
   for (int i = 0; i < 4; ++i)
      substitutions += apply_pv_ps_to_instr(group,
                           prev_group[i],
                           ALU_SRC_PV,
                           unlikely(need_force_pvx) ? 0 : i);

   if (prev_group.has_t())
      substitutions += apply_pv_ps_to_instr(group, prev_group[4], ALU_SRC_PS, 0);
