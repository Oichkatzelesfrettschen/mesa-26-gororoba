...
test_operation_join(void)
{
   ...
   for (uint32_t i = 0; i < count; i++) {
       ...
       * const_fill reaches four: a host transfer route, the two executing
       * RB2D contracts over that same use, and the RB3D clear candidate
       * over a bound color target. */
      assert(routes >= 1);
