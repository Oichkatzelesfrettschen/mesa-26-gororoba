 ...
 *
 * No-submit identity of the reference compute identity carrier pass:
 * the BLAKE3 of the little-endian dword stream
 ...
 * one-page output -- with its
 * dword count.  Every pre-hardware consumer (emitter test, manifest
 * writer, kernel replay, the driver's admission and the harness arms)
 * compares against these literals, so a drift here is a drift of the
 * pass's bytes.
 */
