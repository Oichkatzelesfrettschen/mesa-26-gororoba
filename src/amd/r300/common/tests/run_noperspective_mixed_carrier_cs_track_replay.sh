...
#
# Offline CS-tracking replay of the mixed Smooth/NoPerspective carrier
...
if [ -z "${R3V_CS_TRACK_REPLAY_TOOL:-}" ]; then
    echo "R3V_CS_TRACK_REPLAY_TOOL unset; mixed-carrier CS-track replay not run" >&2
    exit 77
fi
...
case "${result}" in
    ...
    *)
        echo "mixed-carrier cell did not replay verdict=ACCEPT draws=2 passed=2" >&2
        exit 1
        ;;
esac
if ! controls=$("${R3V_CS_TRACK_CONTROLS}" "${R3V_CS_TRACK_REPLAY_TOOL}" \
    ...
    echo "${controls}"
    echo "mixed-carrier cell: a control did not hold" >&2
    exit 1
fi
echo "${controls}"

echo "run_noperspective_mixed_carrier_cs_track_replay: mixed-carrier cell ACCEPT" \
