...
| `r300_rb2d_linear_span.c`<br>`r300_rb2d_linear_span.h` | Linear-span decomposition above the rectangle plan: a dword-aligned byte interval becomes the ordered fill plans that cover it exactly, on a 64-byte carrier row of sixteen words, with every field bound read from `r300_rb2d_fill.h` rather than restated. | `none` | `r300-rb2d-linear-span` | `KEEP_SILICON_CONTRACT` |
