...
| `r300_zmask_clear_plan.c`<br>`r300_zmask_clear_plan.h` | The four-stage ZMASK ladder appended after the depth control cell: depth alone, ownership alone, ZMASK bind and clear with compression off, and the same with FAST_FILL_ENABLE; the bind stages refuse a layout that does not fit and their streams admit under HyperZ ownership and refuse without it. | `native` | `r300-zmask-clear-plan` | `KEEP_SILICON_CONTRACT` |
