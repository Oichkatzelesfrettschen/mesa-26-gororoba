...
| `r300_chipset.c`<br>`r300_chipset.h` | Capability derivation per family over the identity table. | `r300g`, `native` | `r300-chip-identity` | `KEEP_SILICON_CONTRACT` |
| `r300_capabilities.h` | Chip capability record shared by screen and compiler decisions. | `r300g`, `compiler`, `native` | `r300-nir-vs-harness` | `KEEP_SHARED` |
...
| `r300_rb2d_copy.c`<br>`r300_rb2d_copy.h` | RB2D source-to-destination copy plans for whole tiled macroblocks and bounded packed-component byte spans, including ordered relocation roles and exact destination write masks. | `native` | `r300-rb2d-copy` | `KEEP_SILICON_CONTRACT` |
...
| `r300_zb_depth_state.c`<br>`r300_zb_depth_state.h`<br>`r300_zb_depth_control_cell.c`<br>`r300_zb_depth_control_cell.h`<br>`r300_zb_depth_surface.c`<br>`r300_zb_depth_surface.h` | Z-buffer binding/test state, the depth-surface descriptors the cell binds, and the dual-oracle depth control cell. | `native` | `r300-zb-depth-state`, `r300-zb-depth-surface`, `r300-zb-depth-control-cell`, `r300-zb-depth-control-replay` | `KEEP_SILICON_CONTRACT` |
| `r300_zb_combined_clear.c`<br>`r300_zb_combined_clear.h` | API-neutral RS485M D24S8 combined attachment clear plan: packed depth/stencil ARGB8888 fill over the exact tiled storage envelope, with guard and overflow validation. | `native` | `r300-zb-combined-clear` | `KEEP_SILICON_CONTRACT` |
| `r300_zb_depth_discovery.c`<br>`r300_zb_depth_discovery.h`<br>`r300_zb_depth_discovery_cell.c`<br>`r300_zb_depth_discovery_cell.h` | The depth address-discovery experiment: the declared scenario, the three-class allocation partition, the resolver-free observation that reports which physical byte a single-pixel draw moved, and the cell that emits it under a one-pixel scissor. | `native` | `r300-zb-depth-discovery`, `r300-zb-depth-discovery-cell`, `r3v-native-zb-depth-discovery-arming-runner` | `KEEP_SILICON_CONTRACT` |
...
| `r300_zb_depth_layout.c`<br>`r300_zb_depth_layout.h` | Tiled storage extent of one depth surface: the microtile and macrotile pixel dimensions from the R300 tile table, the pitch and stored-row rounding r300_texture_get_stride and r300_texture_get_nblocksy perform, the base alignment a macrotiled surface demands, and the guard ranges that bracket the envelope, plus the row-major address resolver the depth oracle reads a surface through. Separate from the parser footprint, which the kernel measures admission against. | `native` | `r300-zb-depth-layout`, `r300-zb-depth-control-cell` | `KEEP_SILICON_CONTRACT` |
| `r300_zb_aspect_copy.c`<br>`r300_zb_aspect_copy.h` | Packed Z24/S8 aspect-copy decomposition: depth spans preserve stencil bytes, stencil spans preserve depth bytes, and every emitted byte interval remains inside the selected packed words. | `native` | `r300-zb-aspect-copy` | `KEEP_SILICON_CONTRACT` |
| `r300_zb_tile_copy.c`<br>`r300_zb_tile_copy.h` | Whole-macrotile relocation plans for the RS485M parity-dependent microblock permutation, expressed as one, two, or four bit-preserving RB2D spans. | `native` | `r300-zb-tile-copy` | `KEEP_SILICON_CONTRACT` |
| `r300_zmask_layout.c`<br>`r300_zmask_layout.h` | ZMASK RAM layout for one depth level: the ZB_ZMASK_PITCH stride and the 3D_CLEAR_ZMASK dword count as a pure function of row pitch, height, compression block and pipe count, plus the per-family ZMASK RAM budget table, cross-checked point by point against a transcription of r300_setup_hyperz_properties. | `native` | `r300-zmask-layout`, `r300-zmask-clear-plan` | `KEEP_SILICON_CONTRACT` |
| `r300_zmask_clear_plan.c`<br>`r300_zmask_clear_plan.h` | The six-stage ZMASK ladder appended after the depth control cell: depth alone, ownership alone, ZMASK bind and clear with compression off, the same with FAST_FILL_ENABLE, the decompression group adding RD_COMP_ENABLE, and the in-use group adding WR_COMP_ENABLE; the stage table pins each stage's ZB_BW_CNTL word and compression block so rule 8 is unreachable, the compressed-write stage alone admits 8x8 on an explicit request, the bind stages refuse a layout that does not fit or is resolved at another block, and their streams admit under HyperZ ownership and refuse without it. | `native` | `r300-zmask-clear-plan`, `r300-zmask-compressed-qualification` | `KEEP_SILICON_CONTRACT` |
| `r300_zmask_compressed_qualification.c`<br>`r300_zmask_compressed_qualification.h` | The compressed-content qualification sequence as data: nonuniform initialization, a compressed-write draw, a switch away, a compressed read, materialization, the metadata-dependence disable, a resolver read, and an independent oracle comparison, with the discriminator rule that refuses a plan whose every observation the fast-clear substitution reproduces, plus the plan-level prerequisite the ZMASK_COMPRESSED materializer consumes. | `none` | `r300-zmask-compressed-qualification` | `KEEP_SILICON_CONTRACT` |
| `r300_zmask_materialize_plan.c`<br>`r300_zmask_materialize_plan.h` | Whole-image materialization planning for metadata-backed D24/S8 images before ordinary packed-storage consumers or metadata retirement. | `native` | `r300-zmask-materialize-plan` | `KEEP_SILICON_CONTRACT` |
