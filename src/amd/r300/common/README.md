...
| `r300_zmask_layout.c`<br>`r300_zmask_layout.h` | ZMASK RAM layout for one depth level: the ZB_ZMASK_PITCH stride and the 3D_CLEAR_ZMASK dword count as a pure function of row pitch, height, compression block and pipe count, plus the per-family ZMASK RAM budget table, cross-checked point by point against a transcription of r300_setup_hyperz_properties. | `none` | `r300-zmask-layout`, `r300-zmask-clear-plan` | `KEEP_SILICON_CONTRACT` |
| `r300_zmask_clear_plan.c`<br>`r300_zmask_clear_plan.h` | The four-stage ZMASK ladder appended after the depth control cell: depth alone, ownership alone, ZMASK bind and clear with compression off, and the same with FAST_FILL_ENABLE; the bind stages refuse a layout that does not fit and their streams admit under HyperZ ownership and refuse without it. | `none` | `r300-zmask-clear-plan` | `KEEP_SILICON_CONTRACT` |
