...
| `r300_compute_identity_carrier.c`<br>`r300_compute_identity_carrier.h` | The identity verb's raster lowering: the fetched producer pass over a storage-buffer input as F32_4 records into a storage-buffer output as one C4_32_FP slot row, with the FP24 host model of the delivery. | `native` | `r300-compute-identity-carrier`, `r300-compute-identity-carrier-cs-track-replay` | `KEEP_SILICON_CONTRACT` |
| `r300_compute_verb.c`<br>`r300_compute_verb.h` | The finite compute verb ledger: kernel shape, raster unit, exactness class, route status, per-verb gate, refusal classes, and the failure clauses every compute route obeys. | `native` | `r300-compute-verb-ledger` | `KEEP_SILICON_CONTRACT` |
