   ...
   struct r300_tcl_bypass_triangle_ib *out)
{
   ...
   const struct r300_noperspective_q_lane_plan *q_lane =
      params->noperspective_q_lane;
   if (q_lane != NULL &&
       (!params->varying || flat_color0 || params->rs_tex_adj != NULL ||
        params->sampled || carrier != NULL ||
        r300_noperspective_q_lane_plan_validate(q_lane) != 0))
      return -EINVAL;
...
r300_tcl_bypass_triangle_noperspective_q_lane_fs(
   struct r300_fragment_binary *fs)
{
   /* xyz * rcp(w) over interpolator 0, alpha 1. */
   return r300_fragment_binary_init(
      fs, r300_noperspective_q_lane_fs_block,
      sizeof(r300_noperspective_q_lane_fs_block) /
         sizeof(r300_noperspective_q_lane_fs_block[0]),
      R300_NOPERSPECTIVE_Q_LANE_FS_FG_DEPTH_SRC,
      R300_NOPERSPECTIVE_Q_LANE_FS_US_OUT_W,
      "r300-noperspective-q-lane-compiled");
}
...
int
r300_tcl_bypass_triangle_noperspective_q_lane_family_emit(
   uint32_t width, uint32_t height, bool clip_space,
   uint32_t triangle_count,
   ...
   struct r300_tcl_bypass_triangle_ib *out)
{
   if (out != NULL)
      memset(out, 0, sizeof(*out));
   int rc = r300_noperspective_q_lane_plan_validate(plan);
   if (rc != 0)
      return rc;
   uint32_t output_triangle_count = triangle_count;
   if (clip_space) {
                    ...
                    bool clip_space, struct r300_tcl_bypass_triangle_ib *out)
{
   ...
   if (pass->varying && pass->noperspective_q_lane) {
      /* The PM4 is width-independent; the widest plan stands for the
       * shape. */
