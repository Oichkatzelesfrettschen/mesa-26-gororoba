 ...
 *
 * Manifest writer for the public GPU-producer route: retained no-submit
...
main(int argc, char **argv)
{
   ...
   if (r300_r2vb_public_route_reference_compose(&route) != 0) {
      fprintf(stderr, "public route composition failed\n");
      return 1;
   }
   if (r300_r2vb_public_route_validate_reloc_sites(&route) != 0) {
      fprintf(stderr, "public route relocation sites invalid\n");
   ...
   /* The two allocations the attended transport makes: the carrier the
    * producer writes and the consumer fetches, and the triangle's
    * 64-pixel-pitch B8G8R8A8 target with its canary row.
    */
   ...
   int bo_table_len = snprintf(
      ...
      "  ]\n"
      "}\n",
      (unsigned)R300_R2VB_PUBLIC_ROUTE_SLOT_CARRIER, RADEON_GEM_DOMAIN_GTT,
      RADEON_GEM_DOMAIN_GTT, carrier_size_bytes,
      (unsigned)R300_R2VB_PUBLIC_ROUTE_SLOT_COLOR, RADEON_GEM_DOMAIN_GTT,
   ...
   char sites[256];
   int sites_len = 0;
   for (uint32_t i = 0; i < route.reloc_site_count; i++) {
      sites_len += snprintf(&sites[sites_len],
                            sizeof(sites) - (size_t)sites_len,
                            "%s{\"slot\": %u, \"ib_index\": %u}",
                            i == 0 ? "" : ", ", route.reloc_sites[i].slot,
                            route.reloc_sites[i].ib_index);
   }
   ...
   int manifest_len = snprintf(
      ...
      "{\n"
      "  \"schema\": \"r300-r2vb-public-route/1\",\n"
      "  \"emitter\": \"r300_r2vb_public_route\",\n"
      "  \"ib_dwords\": %u,\n"
      ...
      "}\n",
      route.ib_size_dwords, route.consumer_start_dwords, ib_blake3_hex,
      sites, carrier_size_bytes, R300_R2VB_PRODUCER_POISON_DWORD,
      (unsigned)R300_TRIANGLE_COLOR_BYTES, R300_TRIANGLE_COLOR_SENTINEL);
   rc |= write_file(dir, "manifest.json", manifest, (size_t)manifest_len);

   r300_r2vb_public_route_release(&route);

   if (rc == 0) {
      printf("r300_r2vb_public_route_manifest: wrote ib.bin, "
