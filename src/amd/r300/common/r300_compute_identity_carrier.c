                                     ...
                                     struct r300_r2vb_producer_layout *out)
{
   if (out == NULL || record_count < 1 ||
       record_count > R300_COMPUTE_IDENTITY_CARRIER_MAX_RECORDS)
      return -EINVAL;
   ...
   struct r300_r2vb_fetched_producer_ib *out)
{
   ...
   if (rc != 0)
      return rc;
   if (params->carrier_offset % R300_COMPUTE_IDENTITY_CARRIER_OUTPUT_ALIGN !=
          0 ||
       params->source_offset % R300_COMPUTE_IDENTITY_CARRIER_INPUT_ALIGN != 0)
      return -EINVAL;
   ...
   const struct r300_r2vb_fetched_producer_params producer = {
      ...
      .source = {
         ...
         .stride_bytes = R300_COMPUTE_IDENTITY_CARRIER_RECORD_BYTES,
                                       ...
                                       uint32_t out_dwords)
{
   ...
   const struct r300_vertex_stream stream = {
      ...
      .stride = R300_COMPUTE_IDENTITY_CARRIER_RECORD_BYTES,
      .size_bytes =
         (uint64_t)record_count * R300_COMPUTE_IDENTITY_CARRIER_RECORD_BYTES,
   };
   return r300_r2vb_identity_deliver(R300_VERTEX_FORMAT_F32_4, &stream, 0,
                                     record_count, out, out_dwords);
}
