...
/* CONSTFILL orchestrator: out[gid] = C, the degenerate compute-as-raster op.  A
 * constant store has no input to sample, no per-element arithmetic, and no
 * fragment shader -- it is a framebuffer clear.  The constant's four
 * little-endian bytes ride the RGBA8 carrier as unorm channels (the same
 * byte-exact round-trip the identity map proves: round(byte/255 * 255) == byte),
 * the RB3D fast-clear writes them to every pixel, and the existing
 * texture->buffer copy moves the cleared RT into the output ssbo.  Scalar 32-bit
 * fill only (element == 4 bytes); a wider vec4 fill is the FP32x4-carrier
 * generalization. */
bool
                                  ...
                                  const struct r300vk_cmd_bind_descriptor_sets *binds)
{
   ...
   /* CONSTFILL binds a single storage buffer: the output. */
   uint32_t out_binding = pl->const_fill.output_ssbo_binding;
   if (out_binding == 0 && !nth_storage_buffer_binding(set, 0, &out_binding))
      return false;
   const struct r300vk_descriptor *out_desc =
      find_descriptor_by_binding(set, out_binding);
   ...
   /* Scalar 32-bit element only (4 bytes), carried on RGBA8. */
   const unsigned element_bytes =
      pl->const_fill.value_components * (pl->const_fill.value_bit_size / 8u);
   if (element_bytes != 4)
      return false;
   ...
   /* The constant's four little-endian bytes as RGBA8 unorm channels: clearing
    * R=byte0 .. A=byte3 lays [byte0,byte1,byte2,byte3] in memory = the value
    * little-endian, and byte/255 round-trips exactly through unorm.  The raw 32
    * bits transport identically whether the kernel's constant was int or float. */
   const uint32_t cval = pl->const_fill.value[0];
   union pipe_color_union color;
   memset(&color, 0, sizeof(color));
   for (unsigned i = 0; i < 4; i++)
      color.f[i] = (float)((cval >> (8u * i)) & 0xffu) / 255.0f;
   pipe->clear(pipe, PIPE_CLEAR_COLOR0, NULL, &color, 0.0, 0);
   pipe->flush(pipe, NULL, 0);
   ...
   struct pipe_transfer *rt_xfer = NULL;
   const void *rt_map =
      pipe->texture_map(pipe, rt, 0, PIPE_MAP_READ, &copy_box, &rt_xfer);
   if (rt_map) {
      struct pipe_transfer *out_xfer = NULL;
      struct pipe_box out_box;
      memset(&out_box, 0, sizeof(out_box));
      ...
      void *out_bytes = pipe->buffer_map(pipe, out_buf->resource, 0,
                                         ...
                                         &out_box, &out_xfer);
      if (out_bytes) {
         r300vk_identity_map_copy_rows(out_bytes,
                                       width * util_format_get_blocksize(fmt),
                                       rt_map, rt_xfer->stride, width, height,
                                       util_format_get_blocksize(fmt), total);
         pipe->buffer_unmap(pipe, out_xfer);
         ok = true;
      }
      pipe->texture_unmap(pipe, rt_xfer);
