...
At the current cutover boundary the ledger covers 39 files and six named
candidate functions: every file is deleted with the fake Vulkan lifetime, and
no row retains fixture value -- each former fixture has been rebound to its
surviving owner and runs without the fake ICD.  No implementation is classified `MOVE_R300G` or `EXTRACT_COMMON`.
In particular:
...
- `r300-r2vb-typed-route-oracle` (`src/gallium/drivers/r300/tests/`)
  ...
  `r300_r2vb_plan_producer`; `r300-r2vb-producer-census` (same directory,
  same fixture) preserves the former producer census with its exact row and
  RC-statistics format, proves its row table complete against the corpus
  manifest, and carries two calibrated mutations (a corpus member missing
  from the rows, a row naming a module outside the corpus); the fake census's
  check that fragment preparation needs a pipeline layout tested the fake
  preparation itself and leaves with it; the typed-carry corpus lives beside
  both as `src/gallium/drivers/r300/tests/typed_carry_corpus/
  r300_typed_carry_reference.h`, regenerated by its own script;
