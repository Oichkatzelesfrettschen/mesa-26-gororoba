 ...
 *
 * Non-submitting arming runner for the depth control cell: builds the
 * exact two-triangle depth-test cell an attended run would submit,
...
main(int argc, char **argv)
{
   ...
   r3v_native_arming_collect(&facts, vendor_id, device_id,
                             R3V_NATIVE_CELL_KIND_ZB_DEPTH_CONTROL, digest,
                             evidence_dir, kernel, sizeof(kernel), module,
                             sizeof(module));

   printf("r3v native zb-depth-control arming report\n");
   printf("cell_kind=zb-depth-control\n");
   printf("ib_dwords=%u\n", ib_dwords);
   printf("ib_blake3=%s\n", digest);
   printf("  %-22s declared=%-34s observed=%-34s %s\n", "hazard gate",
