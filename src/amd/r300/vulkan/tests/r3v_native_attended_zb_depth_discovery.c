 ...
 *
 * Attended depth address discovery: submits the single-pixel discovery
 * cell to RS485M silicon through the native ICD, retains the depth
 * allocation before and after the draw, and reports which physical byte
 * moved.  This program performs a live DRM_RADEON_CS and runs only under
 * the operator's authorization; the driver's arming conjunction admits
 * it under the cell's own digest, and every stage prints and flushes
 ...
 *
 * The experiment identity is the initial image, not the command stream.
 * The three linear scenarios emit byte-identical IBs and differ in their
 * stencil seed alone, so the retained artifact carries a BLAKE3 over the
 * initial depth image and an IB digest could not separate seed A from
 * seed B.
 */
...
/* One selectable scenario, named on the command line. */
...
#define SCENARIO_OPTION(suffix, descriptor)                                   \
   { #descriptor, R3V_NATIVE_ZB_DISCOVERY_SCENARIO_##suffix },
   R3V_NATIVE_ZB_DISCOVERY_SCENARIOS(SCENARIO_OPTION)
#undef SCENARIO_OPTION
};
...
main(int argc, char **argv)
{
   ...
   const struct r300_zb_depth_discovery_scenario *scenario =
      r3v_native_zb_discovery_scenario_descriptor(chosen_scenario->selection);
   struct r300_zb_depth_layout layout;
   if (scenario == NULL ||
       r300_zb_depth_discovery_layout(scenario, &layout) != 0) {
...
#define LOAD_DEVICE(name) PFN_##name name = (PFN_##name)gdpa(device, #name)
   ...
   result = r3v_native_record_zb_depth_discovery(
      ...
      chosen_scenario->selection, chosen_arm->arm);
   if (result != VK_SUCCESS) {
   ...
   int length = snprintf(
      ...
      "}\n",
      outcome_names[outcome], scenario->name, chosen_arm->name,
      scenario->pixel_x, scenario->pixel_y, scenario->initial_depth_code,
      scenario->initial_stencil, scenario->marker_depth_code, initial_blake3,
