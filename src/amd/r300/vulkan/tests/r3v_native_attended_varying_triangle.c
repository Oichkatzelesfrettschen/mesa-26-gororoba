 ...
 * as two FLOAT_4 elements and the fragment program writes the
 * interpolated varying.  The color-target oracle judges the gradient at
 * the interior samples against the barycentric expectation.  The
 * arming digest is the varying cell's own, which the runner prints
 * beside its recording.
 */
...
/* The reference triangle in NDC: the viewport transform at the 64x64
 * target maps it onto the window-space reference payload, and the
 * varying program computes the reference tint from it.
 */
...
main(int argc, char **argv)
{
   ...
   /* The cell submits on the CPU delivery route: every producer gate
    * stays closed, so a gate naming a producer route refuses before the
   ...
   if (!record_only &&
      ...
      fprintf(stderr,
              "the varying triangle cell runs on the CPU route; every "
   ...
   uint32_t cell_dwords = 0;
   {
      ...
      if (r300_tcl_bypass_triangle_varying_reference_emit(&cell) != 0 ||
          r300_tcl_bypass_triangle_validate_reloc_sites(&cell) != 0) {
         fprintf(stderr, "varying cell emission failed\n");
         return finish(OUTCOME_SUBMISSION_REFUSED);
      }
   ...
   printf("cell varying-triangle ib_dwords=%u ib_blake3=%s\n", cell_dwords,
          cell_digest);
   fflush(stdout);
...
#define LOAD_DEVICE(name) PFN_##name name = (PFN_##name)gdpa(device, #name)
   ...
   /* The clip-space reference triangle: the CPU route transforms it to
    * the window-space reference positions and the vertex program
    * computes the tint per vertex.
    */
   ...
   if (vkCreateShaderModule(
       ...
       vkCreateShaderModule(
          ...
          &(VkShaderModuleCreateInfo){
             .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
             .codeSize = sizeof(r3v_reference_fragment_varying_spirv),
             .pCode = r3v_reference_fragment_varying_spirv,
          },
          NULL, &fs) != VK_SUCCESS) {
   ...
   /* The hazard: the deferred draw executes the varying job into the
    * carrier and a live DRM_RADEON_CS reaches the command processor
    * here.  The submission is one-shot; whatever it returns, no
   ...
   stage("oracle");
   /* The interior expectation is the barycentric interpolation of the
    * reference tint colors -- the values the varying program computes
    * from the NDC triangle -- within the oracle's byte tolerance.
    */
   ...
   r300_tcl_bypass_triangle_varying_extent_oracle(
      ...
      r300_tcl_bypass_triangle_varying_colors, color_map,
   ...
   int length = snprintf(
      ...
      "{\n"
      "  \"schema\": \"r3v-native-varying-triangle-outcome/1\",\n"
      "  \"route\": \"cpu\",\n"
      "  \"verdict\": \"%s\",\n"
      "  \"submit_result\": %d,\n"
      "  \"queue_status\": \"%s\",\n"
      ...
      "  \"exterior_samples\": %u,\n"
      "  \"interior_max_deviation\": %u,\n"
      "  \"interior_tolerance\": %u\n"
      "}\n",
      outcome_names[outcome], submit_result,
      r3v_native_queue_status_name(queue_status), cell_dwords, cell_digest,
