 ...
 *
 * Non-submitting arming runner for the producer-only cell: builds the
 * exact R2VB producer pass an attended run would submit, reports its
 * digest and every arming factor, and stops at the authorization
 * boundary.  The runner performs no ioctl and creates no Vulkan device,
 * so running it is safe on the target host.  The triangle and
 * direct-write cells keep their own runners; each cell's digest
...
main(int argc, char **argv)
{
    ...
    * its freshness is itself an arming factor.  The optional stream
