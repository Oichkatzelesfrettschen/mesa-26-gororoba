...
def main():
    if len(sys.argv) != 3:
        print("usage: r3v_native_public_gpu_producer_arming_runner_check.py "
              "<route-runner> <producer-runner>", file=sys.stderr)
        return 2
    ...
    with tempfile.TemporaryDirectory() as evidence_dir:
        ...
        if "cell_kind=r2vb-gpu-producer-public" not in undeclared.stdout:
            return fail("report does not name the public route cell",
                        undeclared)
        ...
        producer_dwords = re.search(r"^ib_dwords=([1-9][0-9]*)$",
                                    producer.stdout, re.MULTILINE)
        ...
        if producer_digest.group(1) == digest.group(1):
            return fail("the route and its producer half report one "
        ...
        if wrong_cell.returncode == 0 or "MISMATCH" not in wrong_cell.stdout:
            return fail("producer-digest authorization did not refuse on "
                        "the digest factor", wrong_cell)
        if "route: gpu-producer" not in wrong_cell.stdout:
            return fail("report does not name the route under open "
                        "delivery gates", wrong_cell)
        ...
        if half_gated.returncode == 0:
            return fail("a single delivery gate reported an armed route",
                        half_gated)
        if "route: cpu" not in half_gated.stdout:
            return fail("a single delivery gate did not name the CPU "
                        "route", half_gated)
    ...
    print("r3v_native_public_gpu_producer_arming_runner_check: "
          "refusals hold")
    return 0
