...
# authorization, and the wrong-cell refusal when the producer runner's
# digest is declared against the concatenated stream.
...
def main():
    if len(sys.argv) != 3:
        print("usage: r3v_native_reingest_arming_runner_check.py "
              "<reingest-runner> <producer-runner>", file=sys.stderr)
        return 2
    ...
    with tempfile.TemporaryDirectory() as evidence_dir:
        ...
        if "cell_kind=r2vb-reingest" not in undeclared.stdout:
            print("FAIL: report does not name the re-ingest cell",
                  file=sys.stderr)
            print(undeclared.stdout, file=sys.stderr)
            return 1
