 ...
 *
 * Non-submitting arming runner for the direct-write control: builds the
 * exact 2D solid-fill cell an attended control run would submit, reports
 ...
 * so running it is safe on the target host.  The triangle cell keeps its
 * own runner; each cell's digest authorizes only its own stream.
 */
...
/* Builds the fixed control cell and returns its IB digest, the content an
...
main(int argc, char **argv)
{
   ...
   r3v_native_arming_collect(&facts, vendor_id, device_id,
                             R3V_NATIVE_CELL_KIND_DIRECT_WRITE, digest,
                             evidence_dir, kernel, sizeof(kernel), module,
                             sizeof(module));

   printf("r3v native direct-write arming report\n");
   printf("cell_kind=direct-write\n");
   printf("ib_dwords=%u\n", ib_dwords);
   printf("ib_blake3=%s\n", digest);
   printf("  %-22s declared=%-34s observed=%-34s %s\n", "hazard gate",
