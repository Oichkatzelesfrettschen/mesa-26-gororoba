...
#
# Drives the non-submitting depth-control arming runner over a fresh
...
def main():
    if len(sys.argv) != 3:
        print("usage: r3v_native_zb_depth_control_arming_runner_check.py "
              "<depth-control-runner> <triangle-runner>", file=sys.stderr)
        return 2
    ...
    with tempfile.TemporaryDirectory() as evidence_dir:
        ...
        if "cell_kind=zb-depth-control" not in undeclared.stdout:
            print("FAIL: report does not name the depth-control cell",
                  file=sys.stderr)
            print(undeclared.stdout, file=sys.stderr)
            return 1
    ...
    print("r3v_native_zb_depth_control_arming_runner_check: refusals hold")
    return 0
