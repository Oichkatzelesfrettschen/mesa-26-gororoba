                               ...
                               struct terakan_pipeline_compute const *pipeline)
{
   ...
   for (uint32_t m = 0; m < rat_count; ++m) {
      ...
      if (is_image_uav[m]) {
          ...
          * Emit in r600g's canonical order:
                           ...
                           struct terakan_pipeline_compute const *pipeline)
{
    ...
    * direct, base, and indirect launch.  The TGID.z diagnostic path owns
    * START_Z explicitly; this pipeline-dirty state only establishes a safe
    * zero baseline. */
