                               ...
                               struct terakan_pipeline_compute const *pipeline)
{
    ...
    * (SSBO / storage-texel-buffer). Image UAVs require dual
    * SET_RESOURCE emission at CS+160+m for IMMED and CS+168+m for REAL.
    */
   ...
   if (image_uav_count > TERAKAN_MAX_COMPUTE_STORAGE_IMAGES) {
      /* The IMMED/REAL slot windows only hold TERAKAN_MAX_COMPUTE_STORAGE_IMAGES
       * images side by side before an image REAL slot aliases into an earlier
       * image IMMED slot. Refuse the dispatch instead of emitting a corrupt IB.
       */
      fprintf(stderr,
   ...
   /* A store_global fallback path has no per-store RAT id. Bind its RAT0 target
    * to the highest bound mutable descriptor so a read-only resource cannot
    * consume the writable color-export slot.
    */
   if (uav_count == 0) {
   ...
   /* 2. RAT write path.
    *
    * TeraScale-2 compute stores use the color-buffer exporter: each RAT slot M
    * is backed by CB_COLOR{M}_BASE plus companion registers.  Terakan keeps
    * store_ssbo on the uav_instr path, so the intrinsic id_base selects the
    * per-binding RAT slot that this loop publishes.
    *
    ...
    * space with buffer UAVs but consume distinct CS+160+image_m and
    * CS+168+image_m slot windows.
    */
   uint32_t image_m = 0;
   for (uint32_t m = 0; m < rat_count; ++m) {
      ...
      if (is_image_uav[m]) {
         /* Storage-image UAV.
          * Emit in r600g-compatible order:
      ...
      /* Buffer UAV: program CB_COLOR{M} with a synthetic buffer-format
      ...
      /* BUFFER descriptors bake the view offset into color.dim as
       * offset_elements + elements - 1.  The shader adds the same offset
       * through buffer_uav_base_granularity_offset, so CB_COLOR_DIM must
       * permit the final hardware element index. */
