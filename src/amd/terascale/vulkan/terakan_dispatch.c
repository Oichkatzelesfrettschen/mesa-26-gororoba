                               ...
                               struct terakan_pipeline_compute const *pipeline)
{
   ...
   for (uint32_t n = 0; n < res_count; ++n) {
      ...
      *p++ = res_slot * 8;
       ...
       * evergreen_packet3_check, SQ_TEX_VTX_VALID_BUFFER branch) reads
       * ib[WORD0] as a byte offset and writes
       *     ib[WORD0] = reloc->gpu_offset + offset
       * Writing 0 erases any per-element VkDescriptorBufferInfo::offset
       * and collapses shared-BO descriptor arrays (CTS
       * ssbo.readonly.layout.single_struct.single_buffer.*_instance_array)
       * onto a single base.  desc[0] from
       * terakan_descriptor_create_for_storage_buffer is
       * (uint32_t)(buffer->va + buf_info->offset); subtracting bo->va
       * recovers the per-element byte offset for the kernel reloc to
       * resolve.
       *
       * Diagnostic env knobs:
       *   TERAKAN_PROBE_WORD0_SENTINEL=1  -> 0xCAFEBABE (kernel rejects)
       *   TERAKAN_PROBE_WORD0_SENTINEL=2  -> bo->va + 0x1000 sentinel
       *   TERAKAN_VFETCH_FORCE_WORD0_ZERO=1 -> revert to legacy
       *                                       WORD0 = 0 emission. */
      {
   ...
   for (uint32_t m = 0; m < rat_count; ++m) {
       ...
       * buffer_uav_base_granularity_offset[m] to the coord via a
       * KCACHE bank-15 load.  Index m is the compact UAV slot order
       * produced by lowering, while uav_mr_idxs[m] selects the
       * mutable-resource descriptor backing that slot. */
                           ...
                           struct terakan_pipeline_compute const *pipeline)
{
   ...
   /* FIX-W (Q-2026-04-20): select active shader variant.  If production
    * FIX-W is enabled on this pipeline AND the command writer has a
    * bound baseArrayLayer in [0, 8) AND the matching variant compiled
    * successfully, use the variant's shader BO + HW register values.
    * Otherwise fall back to the base shader (which uses the wedged
    * kcache path and is known to fail for this use case). */
   ...
   if (pipeline->fix_w_enabled &&
       command_writer->fix_w_current_layer >= 0 &&
       command_writer->fix_w_current_layer < 8 &&
       pipeline->fix_w_variants[command_writer->fix_w_current_layer] != NULL) {
      active_shader = pipeline->fix_w_variants[command_writer->fix_w_current_layer];
      ...
      if (debug_get_bool_option("TERAKAN_DEBUG_FIX_W", false)) {
         fprintf(stderr,
            "TERAKAN_FIX_W: dispatch selects variant[layer=%d] "
            "program_va_shr8=0x%x\n",
            command_writer->fix_w_current_layer, active_sq_pgm_start_cs);
      }
   }
