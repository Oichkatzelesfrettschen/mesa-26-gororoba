 ...
 * and the import, attach, and destroy entry points.
 *
 * Policy is intentionally closed: every dma-buf import classifies
 * into one of the domains below, and every domain has an explicit
 * support verdict. Adding a new domain or promoting a pending-probe
 * domain requires updating both the enum and the policy table.
 ...
 *
 * INVALID is the explicit zero-initialized sentinel. A
 * terakan_carrier_policy row read from zero-initialized memory must
 * classify as INVALID so accidental zero-init does not silently
 * masquerade as a supported carrier path.
 *
 * - SURFACE_SYNC_ALLOWED:  carrier path is empirically backed on Palm
 *                          and produces a usable carrier.
 * - PENDING_PROBE:         documented by AMD, not yet backed on Palm;
 *                          reject at import.
 * - FORBIDDEN:             explicitly disallowed, for example global
 *                          CAS semantics are not linearizable on Palm.
 * - DECOMPRESS_REQUIRED:   reachable only via an explicit uncompressed
 *                          fallback before export.
 */
enum terakan_carrier_support {
   TERAKAN_CARRIER_SUPPORT_INVALID = 0,
   TERAKAN_CARRIER_SURFACE_SYNC_ALLOWED,
...
struct terakan_dmabuf_carrier {
   ...
   /* Fence sidecars: the dma-buf reservation owns the actual kernel
    * sync state. These FDs are kept open across the carrier lifetime
    * and produced/consumed by acquire/release. */
   int                              acquire_sync_fd;
   int                              release_sync_fd;
};

/*
 * Policy table lookup. Returns a pointer to the immutable per-domain
 * policy entry, or NULL if the domain is out of range.
 */
 ...
 * on. TERAKAN_DISABLE_DMABUF_CARRIER_IMPORT=1 is the emergency kill
 * switch.
 *
 * The export side (locally allocated VkDeviceMemory whose
 * export_handle_types contains DMA_BUF_BIT_EXT) defaults off. Opt in
 * via TERAKAN_ENABLE_DMABUF_CARRIER_EXPORT=1 when an external consumer
 * imports the exported fd.
 *
...
/*
 * Attach a carrier to an existing BO owned by the caller, typically a
 * terakan_device_memory BO. Does not call the winsys import_fd path
 * and does not dup the caller's dma-buf fd; the caller already holds
 * those references via its own owning struct.
 *
 * On SURFACE_SYNC_ALLOWED the carrier is allocated, populated with
 * existing_bo as its non-owning BO pointer, and the BO's atomic
 * carrier pointer is published via release-store. On any other support
 * verdict the function returns VK_ERROR_FEATURE_NOT_PRESENT and *out
 * remains NULL.
 *
 * The BO's atomic carrier field must be NULL at entry; attaching twice
 * without an intervening destroy is a caller bug and asserts in debug
 * builds.
 */
VkResult
 ...
 * Allocates a fresh BO via the winsys PRIME-import contract, publishes
 * the carrier on it, and sets owns_bo = true so destroy frees the BO.
 *
 * Prefer terakan_dmabuf_carrier_attach() when the dma-buf is already
 * imported into a terakan_device_memory; double-importing the same
 * dma-buf into two distinct terakan_bo wrappers is a carrier-association
 * soundness bug.
 */
VkResult
 ...
 * carrier pointer, close carrier-owned sync FDs, free the BO only when
 * owns_bo is true, then free the carrier itself. Safe to call on a
 * partially constructed carrier.
 */
void
