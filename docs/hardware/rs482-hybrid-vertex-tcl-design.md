...
| HBTCL-04f.3 | DONE: the one-FP32-`vec4` split distinguishes float, signed integer, unsigned integer, and boolean carries; integer components enter only when NIR range analysis proves an exact binary32 round trip (`abs(sint) <= 2^24`, `uint <= 2^24`), and every other integer frontier declines. Transport wider than one `vec4` remains HBTCL-04f.5 | HBTCL-04f.2 |
