...
heap. Fix `git-150a16dc47` (on main) caps the r3xx/r4xx byte budget below the
wrap and adds the r5xx alt-count path; validated on RS482 (the 65536-point draw
renders). The R2VB `TCL_BYPASS` leg emits exactly these large-count draws, so it
inherits the clamp -- do not re-introduce an unclamped `NUM_VERTICES` emit.
...
- **Delivery**: a `cb_flush_clean` cache barrier, then `3D_LOAD_VBPNTR`
  ...
  in the previous section. The barrier posts `VAP_PVS_STATE_FLUSH_REG = 0`
  (`0x2284`, `r300_r2vb.c:849`) to synchronize the engine -- a posted write into
  the `0x2200-0x2504` window, which is safe; the hazard is that no register in
  that window is ever *read* (a read wedges the northbridge, see the asymmetry
  above), never that it is written.
...
- **Lighting shares the 64-slot fragment ceiling with the transform, and a slot
  ...
  multi-pass or R400_US route (which lifts ALU and TEX to 512) the transform uses
  for `w_clip` divide and clipping (HBTCL-04f), rather than onto a separate
  lighting engine.
...
registry). RE corpus (steinmarder-r300): the register-opcode-atom inventory
...
underflow finding and its `git-150a16dc47` fix
...
Rocq proofs: `open_gororoba/proofs/theories/IDCT8DP4ExactBound.v`,
