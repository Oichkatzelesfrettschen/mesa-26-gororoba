...
| HBTCL-04f.3R | OPEN: wire the typed split into the production draw route. The admission gate builds the restaged position FS (`r300_r2vb_build_restaged_fs_nir`, the plan consumer in `rs482-producer-alu-compaction-design.md`) and preflight-compiles it, so the backend verdict (`REJECT` unsupported, `OVER_ALU_BUDGET` split-eligible) decides ALU-lowering capability. The pre-lowering `r300_nir_op_is_fragment_aluable` whitelist scan narrows to the structural facts that survive lowering: single-block control flow, plain I/O and uniform/UBO intrinsics, a `gl_Position` output, leading-input feed. A narrow typed-source contract admits first under `R300_R2VB_TYPED_SPLIT`: Boolean `flt -> b2f32`, signed `f2i32` + constant clamp `-> i2f32`, unsigned `max(x,0)` + `f2u32` + constant clamp `-> u2f32`; an under-budget typed producer declines to gallivm (`TYPED_SINGLE_PASS_UNPROVEN`) until proven, so every routed typed shader passes the exact carry contract. Requires a new executable SHA | HBTCL-04f.3, HBTCL-04f.3c, COMP-PLAN-01 |
...
the host-tested typed split primitive whose production-route integration is open
(04f.3R, ahead of it the flat-input epsilon parity 04f.3c), 04f.4-04f.5 the
remaining over-budget producer escape;
...
- The typed one-`vec4` split is a host-tested primitive unreachable through the
  ...
  before RC emission, so route integration (04f.3R) builds and preflights the
  restaged FS instead of the pre-lowering VS, and flat-input epsilon parity
  (04f.3c) precedes it.
