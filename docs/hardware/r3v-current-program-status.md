...
| Deployed kernel source checkpoint | `2be21eaa892723f1c9cd826b7331c7d234e2c1ce`; the receipts below were taken against `0104ede3f1964cc844f9f1839cb6953e2639c4e6` (linux-radeon-gororoba branch `radeon/r300_tcl_bypass_color0_width`, COLOR0 width admission) and hold across the move by the admission identity stated under the table | `radeon-custom` PKGBUILD `_source_commit`; on-target `source-identity.toml` |
| Package version | `radeon-unified-dkms 0.8.13-1` (radeon-custom `9a52df357` PKGBUILD), built for `7.1.8-1-cachyos` and `6.18.42-1-cachyos-lts` with `lockup_timeout=0` from the package-owned board policy; the receipts below were taken under `0.8.12-1`; policy package as `pacman -Q radeon-rs482-policy` reports on the box | `pacman -Q` on the target |
| Loaded module srcversion | `46C05689F2C98A526C314F4`; the receipts below were taken under `729892A3F3530EB12B8D842`, and the exact identity-carrier route record binds to `088E045518D972727C1DD1C`, an earlier module still | `/sys/module/radeon/srcversion` on the target |
...
`replay_r300_cs_track.c`, and `replay_r300_tcl_bypass_ib.c` carry equal
git blob hashes, so every command-stream admission predicate a cell was
judged by is byte-identical. Three commits separate the pins: one docs,
