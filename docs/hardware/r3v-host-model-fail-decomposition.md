...
(`r3v_native_image.c:41-48`, a single `if` over nine ORed field
...
- `vktApiObjectManagementTests.cpp:3605-3618` (`img1D`/`img2D`/`img3D`/
  ...
  (never admitted, guard line 42) with `arraySize=4`; `img3D` sets
  `imageType=3D` (never admitted) with `extent.depth=4`; `imgCube` sets
  `flags=CUBE_COMPATIBLE_BIT` (never admitted, guard line 41, since the
  guard requires `flags==0`); `img2D` sets `usage=SAMPLED_BIT|
  COLOR_ATTACHMENT_BIT` (never admitted -- the admitted color-attachment
  arm requires usage to equal `COLOR_ATTACHMENT_BIT` alone, line 55) and
  `arraySize=12`.
- `vktApiObjectManagementTests.cpp:2379-2390` (`Framebuffer::Resources`):
  ...
  (`r3v_native.h:746-747`); the depth attachment carries
  `usage=DEPTH_STENCIL_ATTACHMENT_BIT`, which the guard's `else` arm
  (`r3v_native_image.c:70-71`) refuses outright since no depth usage is
...
|---|---|---|---|---|
| `r3v_native_image.c:41-48` `r3v_CreateImage`, `tiling`/`format` fields | up to 100 (`pipeline_barrier`); 0 of the 90 `object_management` cases close on this alone | `findMaxRGBA8ImageSize` always requests `VK_IMAGE_TILING_OPTIMAL` and `VK_FORMAT_R8G8B8A8_UNORM` against a `VK_IMAGE_TYPE_2D`, one-mip, one-layer, `flags=0` image; the admitted cells require `VK_IMAGE_TILING_LINEAR` and `R3V_NATIVE_TARGET_FORMAT` (`VK_FORMAT_B8G8R8A8_UNORM`) | Admit a second, `OPTIMAL`-tiled, `R8G8B8A8_UNORM` cell at the same fixed extent and usage vocabulary the `LINEAR` cells already admit (`COLOR_ATTACHMENT_BIT` alone, or `TRANSFER_SRC\|DST` alone); confirming this closes all 100 first needs the exact `usage` `findMaxRGBA8ImageSize`'s `vertex_buffer_stride` callers pass (not traced here) | Host-provable: pure `VkImageCreateInfo` field comparison, no submission |
| `r3v_native_image.c:41-48` `r3v_CreateImage`, `imageType`/`flags`/`usage`/`extent` fields | 90 (`object_management`), across five disjoint sub-populations (see above) | 1D/3D `imageType`, `CUBE_COMPATIBLE_BIT` `flags`, `SAMPLED_BIT` combined usage with `arrayLayers>1`, a 256x256 extent past the 64x64 admitted cell, and depth-usage images -- five distinct unadmitted shapes, detailed above | Each sub-population needs its own new admitted shape (1D support, 3D support, cube support, multi-layer sampled support, a larger or second extent cell, depth-format support); none is a single small PR | Host-provable, but not one mechanism -- see the per-population breakdown above |
...
| 3 | `OpNot` admission in the compute SPIR-V front-end plus a CPU route for the resulting job | 11 | `dEQP-VK.api.object_management.*` (9), `dEQP-VK.api.descriptor_set.*` (2) |
...
| -- | (hypothesis, same format-table root cause as rank 1) | 1 | `dEQP-VK.api.descriptor_set.*` (Amber format query) |
