...
| 3 | `r300: admit OpNot and land the CPU route it needs` | `r300_compute_spirv.c` opcode walk plus `r300_compute_verb.c` row plus a `r300_cpu_compute_job.c` kernel (Table 1, row 4) | `dEQP-VK.api.object_management.*compute_pipeline*` | 10 | A second unary opcode (e.g. `OpSNegate`, already in the 126-141 arithmetic band but still refused) stays outside the admitted grammar and still refuses at the front end |
